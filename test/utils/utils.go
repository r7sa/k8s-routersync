...
func LoadImageToKindClusterWithName(name string) error {
	...
	// // Standart code to load image to kind cluster, but it doesn't work in
	// // kind v0.20.0+ due to the change of container runtime from docker to containerd.
	...
	node := strings.TrimSpace(string(nodesOutput))

	// Save image to tar
	tarFile := "/tmp/" + strings.ReplaceAll(name, "/", "_") + ".tar"
	saveCmd := exec.Command("docker", "save", name, "-o", tarFile)
	...
	loadCmd.Stdin, err = os.Open(tarFile)
	if err != nil {
